...
class Hybrid(commands.Cog):
    ...
    async def create_coordinator(
        ...
        if not member_object:
            return await ctx.send(ctx, content='Could not resolve a valid guild member from your input.')
        ...
        await ctx.send(ctx, content=f'{member_object.mention} has been granted coordinator rights in this guild.')
    ...
    async def delete_coordinator(
        ...
        await ctx.send(ctx, content=f'{member_object.mention}\'s coordinator access has been revoked in this guild.')
    ...
    async def list_coordinators(
        ...
        if not voice_channel:
            return await ctx.send(ctx, content='❌ Could not resolve a valid voice channel.')
        async with self.db_pool.acquire() as conn:
            ...
            ''', voice_channel.id)
        if not rows:
            return await ctx.send(ctx, content=f'ℹ️ No coordinators found for <#{voice_channel.id}>.')
        pages = []
        for row in rows:
            user_id = row['user_id']
    ...
    async def create_developer(
        ...
        async with self.bot.db_pool.acquire() as conn:
            await conn.execute('''
                ...
                updated_at = NOW()
            ''', member_object.id, guild_id)
        await ctx.send(ctx, content=f'{member_object.mention} has been granted developer rights in this guild.')
    ...
    async def delete_developer(
        ...
        async with self.bot.db_pool.acquire() as conn:
            await conn.execute('''
                ...
                WHERE user_id = $1
            ''', member_object.id, guild_id)
        await ctx.send(ctx, content=f'{member_object.mention}\'s developer access has been revoked in this guild.')
    ...
    async def list_developers(self, ctx) -> None:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            ''', guild.id)
        if not rows:
            await ctx.send(ctx, content='No developers are configured in this guild.')
            return
        for row in rows:
            user_id = row['user_id']
            user = guild.get_member(user_id)
    ...
    async def create_moderator(
        ...
        if not isinstance(resolved_channel, discord.VoiceChannel):
            return await ctx.send(ctx, content='Could not resolve a valid **voice** channel from your input.')
        async with self.db_pool.acquire() as conn:
            await conn.execute('''
        ...
        await ctx.send(ctx, content=f'{member_object.mention} has been granted VC moderator access in {resolved_channel.name}.')
    ...
    async def delete_moderator(
        ...
        async with self.db_pool.acquire() as conn:
            await conn.execute('''
                ...
                WHERE user_id = $1
            ''', member_object.id, resolved_channel.id)
        await ctx.send(ctx, content=f'{member_object.mention} has been revoked moderator access in {resolved_channel.name}.')
    ...
    async def list_moderators(
        ...
        async with self.db_pool.acquire() as conn:
            ...
            ''', channel_id)
        if not rows:
            return await ctx.send(ctx, content=f'ℹ️ No moderators found for <#{channel_id}>.')
        pages = []
        for row in rows:
            user_id = row['user_id']
            user = guild.get_member(user_id)
    ...
    async def flags(
        ...
        try:
            ...
            if not rows:
                return await ctx.send(ctx, content=f'✅ No users are flagged for <#{channel_id}>.')
            mentions = [f'<@{row["user_id"]}>' for row in rows]
            message = f'🚩 Users flagged for <#{channel_id}>:\n' + '\n'.join(mentions)
            await ctx.send(ctx, content=message)
        except Exception as e:
            await ctx.send(ctx, content=f'❌ Database error: {e}')
            raise
    ...
    def create_ban_alias(self, command_name: str) -> Command:
        ...
        async def ban_alias(
            ...
            *,
            ...
            if not member_object:
                return await ctx.send(ctx, content='❌ Could not resolve a valid member.')
            if duration_hours == 0:
                if not await is_owner_developer_coordinator(ctx):
                    return await ctx.send(ctx, content='❌ Only coordinators can issue permanent bans.')
                if not reason.strip():
                    return await ctx.send(ctx, content='❌ Reason is required for permanent bans.')
            static_channel_id = self.bot.command_aliases.get(guild_id, {}).get('ban', {}).get(command_name)
            if not static_channel_id:
                return await ctx.send(ctx, content=f'❌ No channel alias mapping found for `{command_name}`.')
            role_id = None
            ...
            async with self.db_pool.acquire() as conn:
                ...
                if not role_id:
                    return await ctx.send(ctx, content=f'❌ No ban role alias mapping found for `{command_name}`.')
            async with self.db_pool.acquire() as conn:
            ...
            if not role:
                return await ctx.send(ctx, content=f'❌ Could not resolve role ID `{role_id}`.')
            try:
                await member_object.add_roles(role, reason=f"Banned from <#{static_channel_id}>: {reason or 'No reason provided'}")
            except discord.Forbidden:
                return await ctx.send(ctx, content='❌ Missing permissions to assign ban role.')
            try:
                async with self.db_pool.acquire() as conn:
                    await conn.execute(
                        '''
    ...
    async def list_bans(
        ...
        if not channel_id or not guild.get_channel(channel_id):
            return await ctx.send(ctx, content='❌ Could not resolve a valid channel.')
        async with self.db_pool.acquire() as conn:
            ...
            ''', guild.id, channel_id)
        if not bans:
            return await ctx.send(ctx, content=f'✅ No active bans found for <#{channel_id}>.')
        embeds = []
        for record in bans:
        ...
        async def mute_alias(
            ...
            *,
            ...
            if member_object.voice and member_object.voice.channel and member_object.voice.channel.id == static_channel_id:
                ...
                await ctx.send(ctx, content=f'{member_object.mention} has been muted in <#{static_channel_id}> with reason {reason}.')
            else:
                await ctx.send(ctx, content=f'{member_object.mention} has been muted in <#{static_channel_id}> with reason {reason}.')
        return mute_alias
    ...
    async def list_mutes(
        ...
        try:
            ...
            return await ctx.send(ctx, content='❌ Invalid channel mention or ID.')
        async with self.db_pool.acquire() as conn:
            ...
            ''', channel_id, guild_id)
        if not records:
            return await ctx.send(ctx, content=f'ℹ️ No users are currently muted in <#{channel_id}>.')
        description_lines = []
        ...
        async def unban_alias(
            ...
            *,
            ...
            try:
                ...
                return await ctx.send(ctx, content='❌ Missing permissions to remove ban role.')
            async with self.db_pool.acquire() as conn:
                await conn.execute('''
            ...
            *,
            ...
            async with self.db_pool.acquire() as conn:
                ...
                if not row or row["source"] != "bot":
                    return await ctx.send(ctx, content=f"❌ {member_object.mention} was not muted by the bot in <#{static_channel_id}>.")
                if member_object.voice and member_object.voice.channel and member_object.voice.channel.id == static_channel_id:
            ...
            if member_object.voice and member_object.voice.channel and member_object.voice.channel.id == static_channel_id:
                await member_object.edit(mute=False)
                await ctx.send(ctx, content=f'{member_object.mention} has been unmuted in <#{static_channel_id}>.')
            else:
                await ctx.send(ctx, content=f'{member_object.mention} is no longer marked as muted in <#{static_channel_id}>.')
        return unmute_alias
    ...
    async def delete_alias(
        ...
        if alias_type.lower() not in {'mute', 'unmute', 'ban', 'unban', 'flag'}:
            await ctx.send('❌ `alias_type` must be either `mute`, `unmute`, `ban`, `unban`, or `flag`.', ephemeral=True)
            return
        if not alias_type:
        ...
        await ctx.send(ctx, content=f'✅ Deleted alias `{alias_name}` from `{alias_type}`.')
    ...
    async def list_aliases(self, ctx, channel: discord.TextChannel = None) -> None:
        ...
        if not aliases:
            await ctx.send(ctx, content='No aliases defined in this guild.')
            return
        pages = []
        if channel:
            channel_id = channel.id
            ...
            for kind in ('mute', 'unmute', 'ban', 'unban', 'flag'):
                ...
                pages.append(embed)
        if not pages:
            await ctx.send(ctx, content='No aliases found.')
            return
        paginator = Paginator(self.bot, ctx, pages)
        await paginator.start()
    ...
    async def role(
        ...
        await ctx.send(f"✅ Associated voice channel {channel.mention} with role {role.mention}.")
    ...
    def create_flag_alias(self, command_name: str) -> Command:
        ...
        async def flag_alias(
            ...
            if not channel_id:
                return await ctx.send(ctx, content=f'❌ No flag alias configured for `{command_name}`.')
            if not user:
                return await ctx.send(ctx, content='❌ You must provide a user ID or mention.')
            if re.fullmatch(r'<@!?\d+>', user):
                user_id = int(re.sub(r'\D', '', user))
            elif user.isdigit():
                user_id = int(user)
            else:
                return await ctx.send(ctx, content='❌ Invalid user ID or mention.')
            select_sql = '''
                SELECT 1 FROM users
            ...
            try:
                async with self.db_pool.acquire() as conn:
                    ...
                    if already_flagged:
                        return await ctx.send(ctx, content=f'ℹ️ <@{user_id}> is already flagged for <#{channel_id}>.')
                    await conn.execute(insert_sql, user_id, channel_id)
                    await ctx.send(ctx, content=f'✅ Flagged <@{user_id}> for channel <#{channel_id}>.')
            except Exception as e:
                await ctx.send(ctx, content=f'❌ Database error: {e}')
                raise
    ...
    async def get_mute_unmute_reason(
        ...
        if not mute_rows and not ban_rows:
            return await ctx.send(ctx, content=f'ℹ️ No mute, unmute, ban, or unban history found for {member_object.mention}.')
        lines = []
        ...
        await ctx.send(ctx, content=content)
