...
class Hybrid(commands.Cog):
    ...
    def create_unban_alias(self, command_name: Optional[str]) -> Command:
        ...
        async def unban_alias_text_command(
            ...
            return await ctx.send(f'{self.get_random_emoji()} {member_obj.mention} has been unbanned from {mention_target}.', allowed_mentions=discord.AllowedMentions.none())
    ...
    async def list_server_muters_app_command(
        ...
        if not await is_owner(interaction, interaction.user.id): return await send(content=f'\U0001F6AB You do not have permission because you are not an Owner.')
        async with self.bot.db_pool.acquire() as conn:
    ...
    async def create_alias_app_command(
        ...
        channel_obj = await self.resolve_chanel_app(interaction, channel)
    ...
    async def cap_app_command(
        ...
        if channel_obj is None:
            return await send(content=f'\U0001F6AB Could not resolve a valid channel or temp room from input: {channel}')
    ...
    async def cap_text_command(
        ...
        *,
        ...
        if is_temp_room:
            ...
            room_display = f'Temp room `{temp_room_name}`'
        else:
        ...
        if original_duration:
            ...
            msg = f'{self.get_random_emoji()} Cap set on {room_display} for {moderation_type} for {duration_str}.'
        await send(msg, allowed_mentions=discord.AllowedMentions.none())
    ...
    async def clear_channel_access_app_command(
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            await conn.execute('''
                UPDATE users
                SET coordinator_room_names = array_remove(
                        ARRAY(
                            SELECT room_name FROM unnest(coordinator_room_names) AS room_name
                            WHERE NOT EXISTS (
                                SELECT 1 FROM command_aliases
                                WHERE channel_id = $1 AND room_name = room_name
                            )
                        )
                    ),
                    moderator_room_names = array_remove(
                        ARRAY(
                            SELECT room_name FROM unnest(moderator_room_names) AS room_name
                            WHERE NOT EXISTS (
                                SELECT 1 FROM command_aliases
                                WHERE channel_id = $1 AND room_name = room_name
                            )
    ...
    # DONE
    @commands.command(name='clear', help='Removes a specific channel ID from all users, including temp-room associations.')
    @is_owner_predicator()
    ...
    async def create_moderator_app_command(
        ...
        room_name = is_temp_room and channel_obj.name
        if not is_owner_or_dev:
            async with self.bot.db_pool.acquire() as conn:
                if room_name != '':
                    row = await conn.fetchrow('''
    ...
    async def list_bans_app_command(
        ...
        channel_obj = await self.resolve_chanel_app(interaction, target)
    ...
    async def stage_create_app_command(
        ...
        send = lambda **kw: interaction.response.send_message(**kw, ephemeral=True)
        channel_obj = await self.resolve_chanel_app(ctx, channel)
    ...
    async def list_developers_app_command(
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            if target is None:
                ...
                if member_obj.id!=interaction.user.id: return await send(content=f'\U0001F6AB You do not have permission to use this command (`devs`) for {member_obj.mention}.')
    ...
    async def list_moderators_app_command(
        ...
        _,is_coord=await check_owner_dev_coord_app(interaction,channel_obj)
        if is_owner_or_dev and member_obj:
            if member_obj.id!=interaction.user.id and not is_coord and not is_owner_or_dev: return await send(content=f'\U0001F6AB You do not have permission to use this command (`mods`) for {member_obj.mention}.')
    ...
    async def mark_temp_room_app_command(
        ...
        await send(content=f'{self.get_random_emoji()} Channel {room.mention} has been marked as a temporary room with owner <@{owner_snowflake}>.',ephemeral=True)
    ...
    async def mark_temp_room_text_command(
        ...
        await send(content=f'{self.get_random_emoji()} Channel {channel_obj.mention} has been marked as a temporary room with owner <@{owner_snowflake}>.')
    ...
    async def stage_mute_app_command(
        ...
        member_obj = await self.resolve_member_app(ctx, member)
        if not member_obj:
        ...
        highest_role, success = await check_block(interaction, member_obj, stage_channel)
        async with self.bot.db_pool.acquire() as conn:
            stage = await conn.fetchrow(
            ...
            try:
                ...
                await send(content=f'{self.get_random_emoji()} {member_obj.mention} has been {"muted" if member_obj.voice.mute else "unmuted"}.')
            except Exception as e:
                logger.warning(f'Failed to toggle mute: {e}')
                await send(content=f'\U000026A0\U0000FE0F Failed to toggle mute for {member_obj.display_name}.')
    ...
    async def stage_mute_text_command(
        ...
        if not member_obj:
            return await send(content='\U0001F6AB Invalid member.')
        stage_channel = member_obj.voice.channel if member_obj.voice else await self.resolve_channel(ctx, None)
        if not stage_channel:
            return await send(content='\U000026A0\U0000FE0F No stage channel found.')
        is_owner_or_dev, _ = await check_owner_dev_coord(ctx, stage_channel)
        highest_role, success = await check_block(ctx, member_obj, stage_channel)
        async with self.bot.db_pool.acquire() as conn:
            stage = await conn.fetchrow(
                'SELECT initiator_id FROM active_stages WHERE guild_id=$1 AND channel_id=$2',
                guild_id, stage_channel.id
    ...
    async def stage_promote_app_command(
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            if not is_owner_or_dev or is_coord_or_mod:
                return await send(content=f'\U0001F6AB {member_obj.mention} does not have permission to moderate a stage in this channel')
            highest_role, success = await check_block_app(interaction, member_obj, channel_obj)
            if member_obj.id == stage['initiator_id']:
            ...
            is_coordinator = await conn.fetchval('''
                SELECT 1 FROM stage_coordinators
                WHERE guild_id=$1 AND channel_id=$2 AND discord_snowflake=$3
            ''', guild_id, channel_obj.id, member_obj.id)
            try:
                if is_coordinator:
                    await conn.execute('''
                    ...
                    ''', guild_id, member_obj.id, channel_obj.id)
                    if member_obj.voice and member_obj.voice.mute:
                        await member_obj.edit(mute=False)
                    await send(content=f'{self.get_random_emoji()} {member_obj.mention} promoted to stage coordinator.')
            except Exception as e:
    ...
    async def stage_promote_text_command(
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            if not is_owner_or_dev or is_coord_or_mod:
                return await send('\U0001F6AB {member_obj.mention} does not have permission to moderate a stage in this channel')
            highest_role, success = await check_block(ctx, member_obj, channel_obj)
            if member_obj.id == stage['initiator_id']:
                return await send(content='\U0001F6AB Cannot change initiator role.')
            is_coordinator = await conn.fetchval('''
            ...
            try:
                ...
                logger.warning(f'Failed to toggle promotion: {e}')
                await send(content=f'\U000026A0\U0000FE0F Failed to toggle promotion for {member._objdisplay_name}.')
    ...
    async def stage_survey_app_command(
        ...
        send = lambda **kw: interaction.response.send_message(**kw, ephemeral=True)
        channel_obj = await self.resolve_channel_app(ctx, channel)
        if not isinstance(channel_obj, (discord.VoiceChannel, discord.StageChannel)):
            return await send(content='\U0001F6AB Please specify a valid voice or stage channel.')
    ...
    async def undo_cap_interaction(
        ...
        if is_temp_room:
            original_duration = await self.get_cap(-1, interaction.guild.id, moderation_type, temp_room_name)
            await self.set_cap(-1, interaction.guild.id, moderation_type, None, temp_room_name)
            room_display = f'Temp room `{temp_room_name}`'
        else:
            original_duration = await self.get_cap(channel_obj.id, interaction.guild.id, moderation_type)
            await self.set_cap(channel_obj.id, interaction.guild.id, moderation_type, None)
            room_display = channel_obj.mention
        if original_duration:
    ...
    async def undo_cap_text_command(self, ctx: commands.Context, channel: Optional[str]=commands.parameter(default=None, description='Tag a channel or include its snowflake ID'), moderation_type: Optional[str]=commands.parameter(default=None, description='One of: `mute`, `ban`, `tmute`')):
        ...
        if channel_obj is None: return await send(content=f'\U0001F6AB Could not resolve a valid channel or temp room from input: {channel}.')
        valid_types={'mute','ban','tmute'}
        ...
        if is_temp_room:
            original_duration=await self.get_cap(-1,ctx.guild.id,moderation_type,temp_room_name)
            await self.set_cap(-1,ctx.guild.id,moderation_type,None,temp_room_name)
            room_display=f'Temp room `{temp_room_name}`'
        else:
            original_duration=await self.get_cap(channel_obj.id,ctx.guild.id,moderation_type)
            await self.set_cap(channel_obj.id,ctx.guild.id,moderation_type,None)
            room_display=channel_obj.mention
    ...
    async def delete_moderator_app_command(
        ...
        async with self.bot.db_pool.acquire() as conn:
            ...
            has_remaining_rooms = bool(remaining_rooms)
            if not has_remaining_guild_channels and not has_remaining_rooms:
                return await send(content=f'{self.get_random_emoji()} {member_obj.mention} has had all moderator access revoked in {channel_obj.mention} and {guild.name} (no remaining channels or temp rooms).')
            else:
    ...
    @app_commands.describe(channel='Tag a channel or include its snowflake ID')
    @is_owner_app_predicator()
    async def room_unmute_all_app_command(
        self,
        interaction: discord.Interaction,
        channel: Optional[str] = None
    ):
    ...
    @is_owner_predicator()
    async def undo_room_mute_text_command(
        self,
        ctx: commands.Context,
        channel: Optional[str] = commands.parameter(default=None, description='Tag a channel or include its snowflake ID'),
    ) -> None:
    ...
    async def resolve_chanel_app(self, interaction: discord.Interaction, value: Optional[Union[int, str, discord.TextChannel, discord.VoiceChannel]]) -> Optional[Union[discord.TextChannel, discord.VoiceChannel]]:
        try:
            if isinstance(value, (discord.TextChannel, discord.VoiceChannel)):
                logger.debug(f"Direct channel: {value.id}")
                return value
