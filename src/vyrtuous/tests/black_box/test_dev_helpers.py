...
async def dev_cleanup(guild_id: Optional[int], privileged_author_id: Optional[int]):
...
async def dev_initiation(guild_id: Optional[int], privileged_author_id: Optional[int]):
    bot = DiscordBot.get_instance()
    async with bot.db_pool.acquire() as conn:   
        await conn.execute('''
