''' test_dev_command.py The purpose of this program is to black box test the developer toggle command.
...
async def test_dev_command(bot, voice_channel_one, guild, privileged_author, prefix: Optional[str], not_privileged_author, command: Optional[str], member_ref):
    ...
    captured = await prepared_command_handling(author=privileged_author, bot=bot, channel=voice_channel_one, cog="OwnerCommands", content=formatted, guild=guild, isinstance_patch="vyrtuous.cogs.owner_commands.isinstance", prefix=prefix)
