...
async def test_track_tracks_command(
    ...
    if command == "track":
        match target_type:
            case "all":
                formatted = (
                    f"{command} {text_channel.id} {action} {target_type}".strip()
                )
            case "channel":
    ...
    if message_type == "success":
        ...
        if ref_text:
            assert any(
                str(text_channel_value) in content
                for text_channel_value in text_channel_values
            )
        if ref_channel:
            assert any(
                str(voice_channel_value) in content
                for voice_channel_value in voice_channel_values
            )
        if ref_member:
            assert any(
                str(not_privileged_author_value) in content
                for not_privileged_author_value in not_privileged_author_values
            )
