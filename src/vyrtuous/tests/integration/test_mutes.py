...
@pytest.mark.parametrize(
    ...
    [
        ("Moderator", "!mutes", "all"),
        ("Moderator", "!mutes", "{channel_snowflake}"),
        ("Moderator", "!mutes", "<#{channel_snowflake}>"),
        ("Moderator", "!mutes", "{guild_snowflake}"),
        ("Moderator", "!mutes", "{member_snowflake}"),
        ("Moderator", "!mutes", "<@{member_snowflake}>"),
    ],
)
async def test_voice_mutes(bot, command: str, target, permission_role):
    """
    ...
    if os.environ["TEST_MODE"].lower() == "integration":
        ...
        with ExitStack() as stack:
            ...
            async with capture_command() as end_results:
                command = await mod_commands.list_mutes_text_command(ctx, target=t)
