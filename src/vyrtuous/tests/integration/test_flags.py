...
COMMAND = "flags"
BASE_PERMISSIONS = ["command.info.flags"]
TABLE_NAME = Flag.__tablename__
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("<#{channel_snowflake}>", None, ["command.info.scope.channel"]),
        ("{guild_snowflake}", None, ["commands.info.scope.guild"]),
...
async def test_flags_text_command(
    bot,
    prefix: str,
    target: str | None,
...
async def test_flags_app_command(
    bot,
    target: str | None,
    other_guild: str | None,
    extra_permissions: list[str],
...
async def test_active_flags_database_table(
