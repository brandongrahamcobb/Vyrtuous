...
@pytest.mark.parametrize(
    ...
    [
        ("Sysadmin", "!temps", "all"),
        ("Administrator", "!temps", "{channel_snowflake}"),
        ("Administrator", "!temps", "<#{channel_snowflake}>"),
        ("Administrator", "!temps", "{guild_snowflake}"),
    ],
)
async def test_temps(bot, command: str, target, permission_role):
    """
    ...
    if os.environ["TEST_MODE"].lower() == "integration":
        ...
        with ExitStack() as stack:
            ...
            async with capture_command() as end_results:
                command = await admin_commands.list_temp_rooms_text_command(
                    ctx, target=t
                )
