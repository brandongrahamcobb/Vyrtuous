...
class AliasInformation:
    ...
    async def build(
        ...
        args = (
            message.content[len(bot.config["discord_command_prefix"]) :]
            .strip()
            .split()
        )
        alias_name = args[0]
        ...
        for field, tuple in kwargs.items():
            if field == "duration":
                duration = DurationObject(tuple[1]) if tuple[1] is not None else DurationObject("8h")
                ...
                if not hasattr(cap, "duration"):
                    AliasInformation.information["cap_duration"] = DurationObject("8h").to_seconds()
                else:
                    AliasInformation.information["cap_duration"] = cap.duration_seconds
                if (
                    duration.to_timedelta().total_seconds() > AliasInformation.information["cap_duration"]
                    or duration.number == 0
                ):
                    if AliasInformation.information["executor_role"] == "Moderator":
                        duration_str = DurationObject.from_seconds(AliasInformation.information["cap_duration"])
                        AliasInformation.information["duration"] = duration_str
                        raise DurationError(information=AliasInformation.information)
                AliasInformation.information["expires_in"] = datetime.now(timezone.utc) + duration.to_timedelta()
            if field == "member":
