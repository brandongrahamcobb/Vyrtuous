...
class Moderator:
    ...
    @classmethod
    async def fetch_discord_snowflakes_for_channel_id(cls, channel_id: Optional[int], guild_id: Optional[int]):
        bot = DiscordBot.get_instance()
        async with bot.db_pool.acquire() as conn:
            rows = await conn.fetch('''
    ...
    async def fetch_all_members_in_guild(cls, guild_id: Optional[int]):
        ...
        async with bot.db_pool.acquire() as conn:
            ...
            for row in rows:
                moderators.append(
                    Moderator(channel_id=row['channel_snowflake'],guild_id=guild_id,member_id=row['member_snowflake'])
                )
            return moderators
    ...
    async def grant(cls, channel_id: Optional[int], guild_id: Optional[int], member_id: Optional[int]):
        moderator = Moderator(channel_id=channel_id, guild_id=guild_id, member_id=member_id)
        await moderator.set_channel_id_for_member()