...
class Game:
    ...
    async def moderate_name(self, member: discord.Member, ctx=None) -> bool:
        if not self.config.get('openai_chat_moderation', False) or self.predicator.is_spawd(ctx):
            return False
        try:
            async for moderation_completion in create_moderation(input_array=[ctx.author.name]):
                try:
